...
ONBOARD_STEPS = [
    ...
    {"key": "gmail",      "prompt": "Last step — connect your Gmail so I can send your daily digest and read your replies."},
]
...
async def chat(request: Request):
    ...
    if step_num < len(ONBOARD_STEPS):
        ...
        if key == "email":
            if "@" not in message:
                return JSONResponse({"reply": "That doesn't look like an email. Try again.", "step_num": step_num, "action": None})
            existing = get_user_by_email(message.lower())
            if existing:
                sess["user_id"] = existing["id"]
                upsert_session(sid, sess)
                resp = JSONResponse({"reply": f"Welcome back, {existing['name']}! Redirecting you to your dashboard...", "step_num": step_num, "action": "redirect:/dashboard"})
        ...
        if key == "resume":
            ...
            reply = ONBOARD_STEPS[-1]["prompt"]
            return JSONResponse({"reply": reply, "step_num": step_num + 1, "action": action})
        ...
        if next_step_num >= len(ONBOARD_STEPS):
            ...
            upsert_session(sid, sess)
            resp = JSONResponse({"reply": "You're all set! Redirecting to your dashboard...", "step_num": next_step_num, "action": "redirect:/dashboard"})
            set_session(resp, sid)
            return resp
    ...
    return JSONResponse({"reply": "Onboarding complete! Head to your dashboard.", "step_num": step_num, "action": "redirect:/dashboard"})
...
async def resume_upload(request: Request, file: UploadFile = File(...)):
    ...
    resp = JSONResponse({"reply": "Resume uploaded! Last step — connect your Gmail so I can send your daily digest.", "step_num": step_num + 1, "action": "show_gmail_button"})
    set_session(resp, sid)
    return resp
...
async def settings_post(request: Request):
    ...
    criteria = {
        ...
        "min_salary": int(form.get("min_salary") or 0) or None,
        "max_salary": int(form.get("max_salary") or 0) or None,
...
async def run_digest_for_user(user_id: int):
    ...
    all_jobs = []
    gh_companies = criteria.get("greenhouse_companies", [])
    lv_companies = criteria.get("lever_companies", [])
    job_titles = criteria.get("job_titles", [])
    locations = criteria.get("locations", [])

    for company in gh_companies:
        try:
            jobs = await asyncio.to_thread(fetch_greenhouse_jobs, company)
    ...
    for company in lv_companies:
        try:
            jobs = await asyncio.to_thread(fetch_lever_jobs, company)
            all_jobs.extend(jobs)
        except Exception:
            pass
    ...
    if job_titles:
        try:
            jobs = await asyncio.to_thread(fetch_indeed_jobs, job_titles, locations)
            all_jobs.extend(jobs)
        except Exception:
            pass
